...
def print_devices(page_data):
    if page_data is None:
        print("No result object returned")
        return

    devices = page_data.get("devices", [])
    total = page_data.get("total", len(devices))

    print(f"Returned {len(devices)} of {total} matching devices")
    for device in devices:
        print(
            "{ipAddress:15} {hostname:30} {adapterId}".format(
                ipAddress=device.get("ipAddress") or "",
                hostname=device.get("hostname") or "",
                adapterId=device.get("adapterId") or "",
            )
        )
