...
class NetLDClient {
    ...
    [object] GetConfigurationHistory(
        [string[]]$networks,
        [string]$scheme,
        [string]$data,
        [int]$offset,
        ...
        return $this.Call("Configuration.retrieveConfigHistory", @{
            pageData = @{
                offset = $offset
                pageSize = $pageSize
                total = 0
    ...
    [object] RetrieveRevision(
        [string]$network,
        [string]$ipAddress,
        [string]$configPath,
        [long]$timestamp
    ) {
        return $this.Call("Configuration.retrieveRevision", @{
            network = $network
            ipAddress = $ipAddress
...
function ConvertFrom-RevisionContent {
    ...
    try {
        $bytes = [Convert]::FromBase64String($Revision.content)
    }
    catch {
        throw [NetLDError]::new("Revision content is not valid Base64.")
    }

    if (-not ([string]$Revision.mimeType).StartsWith("text/")) {
        return $null
    }
    ...
    try {
        ...
        throw [NetLDError]::new("Text revision content is not valid UTF-8.")
    }
}
