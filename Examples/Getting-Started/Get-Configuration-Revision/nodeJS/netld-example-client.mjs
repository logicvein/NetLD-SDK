...
export class NetLDClient {
  ...
  retrieveRevision({ network, ipAddress, configPath, timestamp }) {
...
export function decodeRevisionContent(revision) {
  if (!revision.content) {
    return null;
  }

  const compactContent = revision.content.replace(/\s/g, "");
  const decoded = Buffer.from(compactContent, "base64");
  if (decoded.toString("base64").replace(/=+$/, "") !== compactContent.replace(/=+$/, "")) {
    throw new NetLDError("Revision content is not valid Base64.");
  }

  if (!(revision.mimeType || "").startsWith("text/")) {
    return null;
  }
