...
class NetLDClient {
    ...
    [object] GetDeviceInterfaces([string]$network, [string]$ipAddress) {
        return $this.Call("Inventory.getDeviceInterfaces", @{
            network = $network
            ipAddress = $ipAddress
        })
    }
...
function Show-DeviceInterfaces {
    param([object]$Interfaces)

    if ($null -eq $Interfaces) {
        Write-Host "No interface result returned"
        return
    }

    ConvertTo-Json -InputObject @($Interfaces) -Depth 12
}
