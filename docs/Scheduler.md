...
See the [Scheduler Objects](#scheduler-objects) section for a description of the various objects consumed and returned by these APIs.
...
*Most* (but not all) jobs share a common set of "device resolution" parametersused to specify the set of devices that the job applies to (see [Device Resolution Parameters](#device-resolution-parameters))
...
* The *only* job parameters required for this job are the ([Device Resolution Parameters](#device-resolution-parameters)).
...
In version 14.06 [Scheduler.addJob](#scheduler.addjob) is not supported for this job type.  Only [Scheduler.runExistingJobNow](#scheduler.runexistingjobnow) is currently supported.  This means that the SmartChange ("bulk update") must first be created through the browser user interface.

| Name             | Type     | Value Description      |
| ---------------- | -------- | --------------------   |
| replacementMode | String   | Valid values are: "perdevice" or "perjob". |
| templateXml     | String   | This string property should be copied verbatim from the ``JobData`` object for the SmartChange, retrieved from the [Scheduler.getJob](#scheduler.getjob) API. |
...
The "perdevice" replacements XML is similar to the "perjob" XML, with two notable exceptions.  The ``<config>`` tag must now contain a *"device"* attribute whose value is the IP address of the device, followed by an ``@`` character, and finally a managed network name.  Note that if the SmartChange job definition was created in a network called "Headquarters", then a device that is defined to be in another networks, e.g. *"10.0.0.1@Default"*, will be ignored.
